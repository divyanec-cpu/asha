...
- ~~**In-app timed test mode.**~~ **Shipped 2026-08-03.** ASHA runs the section clock while the student works through a mock they already own, capturing per-question time and attempt order as they happen. This is what makes `timing_source = 'measured'` real, and it also finally populates `question_attempts.order_index`, which no earlier flow captured. **No analytic changes** — that is the point of the provenance column.
- ~~**GMAT and MAT configs.**~~ **Shipped 2026-08-03.** Seed data only: exam rows, `exam_configs` marking per pattern, sections, and a taxonomy per exam. Each pattern must be independently verified before seeding, because a wrong marking rule silently corrupts every figure for that exam.

  **That verification rule earned its place.** Most published MAT patterns are out of date — 200 questions / 40 per section / 150 minutes, with a fifth section called "Indian & Global Environment". AIMA gives 150 / 30 / 120 minutes and that section is now "Economic & Business Environment". Seeding the popular figures would have made every MAT accuracy denominator a third too large and looked entirely plausible doing it.

  **Both exams are seeded `active = false`, and GMAT must stay that way until one thing is done.** GMAT Focus is adaptive with IRT scoring (205–805); it has **no per-question marks**, but `exam_configs` requires the marking columns. They hold `1 / 0 / 0`, which makes `marks_earned` a *raw count of correct answers* — honest as a count, and not a score. Before flipping `active` for GMAT, audit every screen rendering a marks figure for wording that would present that count as a score. See `docs/decisions.md`.
...
- **Practice delivery shipped (2026-08-04).** ASHA can now put questions on screen, time them, and mark them against its own key — migration `0009`, `/practice`, and **43 verified original QA questions across three papers**: a 14-question warm-up, a 31-question *coverage* set (one question per CAT QA type, deliberately not all hard, so it finds weak topics fast), and a 12-question *challenge* set at real CAT difficulty whose every wrong option encodes a specific mistake. The tiering exists because a single mixed set does both jobs badly: hard questions are poor at telling you whether someone knows a topic at all, and an easy set flatters.
- **Full-length mocks work (2026-08-06).** `ASHA Mock 1` — 66 questions, VARC → DILR → QA, 40 minutes each. A mock is modelled as **three sequential single-section runs sharing one attempt**, because CAT locks sections; `start` creates a `section_attempt` per section and each submit hands over the next. **Which section is next is derived server-side from which siblings already hold answers**, so a replayed request cannot skip one or reopen a finished one, and the attempt stays `is_complete = false` until every section is sat — which is what keeps a half-finished mock out of the analytics. It is 66 rather than 68 because DILR sets are indivisible; the shortfall is printed by the seed and written onto the paper card. **`npm run seed:all` must be used** — the mock's `paper_items` point at items the content seeds delete and recreate, so running one alone silently empties it.
- **Practice content totals 115 original questions across 8 papers (2026-08-06):** QA 43 (warm-up / coverage / challenge), VARC 24 (4 passages + 8 verbal ability — a full CAT section), DILR 48 (12 sets over 3 papers). **Every CAT question type and all 12 DILR archetypes now have content.**
- **Charts are real charts (2026-08-05, migration `0010`).** `question_stimuli.chart_spec` plus `kind = 'chart'`, rendered as SVG by `StimulusChart.tsx`. A table of exact values is **not** an acceptable stand-in for a chart: it tests arithmetic rather than reading, so tagging one `DILR.ARCH.CHART` would put a false archetype on every attempt row derived from it. **The spec is structured data, never stored SVG markup** — markup rendered via `dangerouslySetInnerHTML` becomes an injection vector the moment the reserved `private` content kind turns writable. **Chart marks carry no printed values**, so chart questions must tolerate gridline-level reading precision.
- **DILR content shipped (2026-08-05).** Seven original sets — arrangement, truth-tellers, caselet, round robin, scheduling, Venn, road network — with 28 questions, and set exhibits that keep their tabular structure. Each set carries its own solver (permutation enumeration, all 3⁶ tournament outcomes, Dijkstra) and **all four logic sets are asserted to have exactly one solution**. Structural assertions also require Venn regions to partition the surveyed total, every network town to be reachable, and one set's questions to stay contiguous within a paper. **Practice DILR is question-level, deliberately unlike DILR logging:** in a practice run ASHA holds the questions and times each one, so it captures per-question outcomes and working order, which is strictly more than replaying a mock can obtain. The archetype still gets recorded, on the exhibit rather than a `set_attempts` row. **This un-reserves `DILR.SKILL.*`** — four nodes reserved since v1 because no DILR question rows existed. **Every key is computed by solving the set, and both logic sets are asserted to have exactly one solution** — which caught a "what if" question that had two valid answers while its hand-written explanation claimed one. An ambiguous set is worse than a wrong one: it looks fine and marks a correct student wrong.
- **VARC content and passage rendering shipped (2026-08-04).** Three original passages plus 16 questions covering all 7 RC and all 4 VA types; the runner now renders a shared `question_stimuli` passage, which no earlier flow could. **Two kinds of answer key, not equally sound, and the difference is documented rather than hidden:** verbal-ability keys are *computed from the construction* (the seed derives the jumble order and asserts it rebuilds the paragraph), while RC keys rest on judgement and are instead anchored to a `support` string that must appear **verbatim** in the passage — which makes them auditable by someone who has never seen a CAT paper. **One seed script owns one `content_sources` row**; both scripts delete-then-insert, so a shared source would mean re-running one silently destroyed the other's items. **Practice runs are excluded from the mock count and the cross-mock trend**: a 14-question set scores out of 42 against a mock's 204, and blending them would produce a trend figure with no meaning. Whether to blend the *question-level* data into per-type analytics is an open builder decision, deliberately not defaulted.
